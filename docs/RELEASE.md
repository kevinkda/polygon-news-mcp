...
`polygon-news-mcp` follows the same release discipline as
`schwab-marketdata-mcp` and `sec-edgar-mcp`.
...
2. Bump `__version__` in `src/polygon_news_mcp/__init__.py` and
   `pyproject.toml`.
