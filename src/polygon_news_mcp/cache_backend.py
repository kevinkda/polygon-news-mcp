"""Pluggable cache backends for polygon-news-mcp (v0.7 T0 template).
...
* :class:`ClickHouseBackend` — the **opt-in** backend.  Enabled only when
  ``pip install polygon-news-mcp[clickhouse]`` is present *and*
  ``POLYGON_CLICKHOUSE_URL`` is configured.  ``clickhouse_connect`` is
...
* :func:`get_cache_backend` — the factory.  Selects the backend from
  ``POLYGON_CACHE_BACKEND`` (``memory`` | ``clickhouse``, default
  ``memory``).
...
ENV_CACHE_BACKEND: Final[str] = "POLYGON_CACHE_BACKEND"
ENV_CLICKHOUSE_URL: Final[str] = "POLYGON_CLICKHOUSE_URL"
...
REQUIRES_CLICKHOUSE_HINT: Final[str] = (
    ...
    "Install it with `pip install polygon-news-mcp[clickhouse]` and set "
    "POLYGON_CLICKHOUSE_URL plus POLYGON_CACHE_BACKEND=clickhouse. "
...
class CacheBackend(Protocol):
    ...
    * **Derived-analysis history** (``append_timeseries`` /
      ``query_timeseries``) — persist time series for derived analytics.
      Only the ClickHouse backend persists; the memory backend returns a
...
class ClickHouseNotInstalledError(RuntimeError):
    ...
    def __init__(self, original: Exception | None = None) -> None:
        self.hint = (
            ...
            "`pip install polygon-news-mcp[clickhouse]`."
        )
...
class ClickHouseBackend:
    ...
    ``POLYGON_CLICKHOUSE_URL`` is configured.  Connection / query
    ...
    _RESPONSE_TABLE: Final[str] = "polygon_response_cache"
    _TIMESERIES_TABLE: Final[str] = "polygon_timeseries"
...
def get_cache_backend() -> CacheBackend:
    ...
    Honors ``POLYGON_CACHE_BACKEND`` (``memory`` | ``clickhouse``).
