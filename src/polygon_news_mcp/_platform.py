...
Mirrors ``schwab_marketdata_mcp._platform`` so polygon-news-mcp shares the
same Windows porting baseline (Sprint A).
"""
...
log = logging.getLogger("polygon_news_mcp._platform")
...
def file_mode(path: Path) -> int:
    ...
    comparable to ``0o600``.  Prefer :func:`is_secure_perms` for the
...
def _notify_windows(title: str, message: str) -> None:  # pragma: no cover - windows-only branch
    ...
    try:
        ...
        notification.notify(title=title, message=message, app_name="Polygon News MCP", timeout=5)
        return
    ...
    script = (
        ...
        '[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Polygon News MCP")'
