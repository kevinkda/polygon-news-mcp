...
1. A lazily-constructed :class:`PolygonClient` (so tests can override the
...
from ..client import PolygonClient, make_client

_lock = asyncio.Lock()
_client: PolygonClient | None = None


async def get_client() -> PolygonClient:
    """Lazily instantiate the Polygon client (one per process)."""
    global _client
    if _client is None:
        async with _lock:
            if _client is None:
...
async def set_client_for_tests(client: PolygonClient | None) -> None:
...
async def call_with_cache(
    fetch: Callable[[PolygonClient], Awaitable[dict[str, Any]]],
    *,
